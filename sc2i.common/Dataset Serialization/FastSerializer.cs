...
namespace sc2i.common
{
	...
	public sealed class SerializationWriter: BinaryWriter
	{
		...
		private const int OptimizationFailure32BitValue = Int32.MaxValue; // 0x10000000

		// The long at which optimization fails because it takes more than 8 bytes
		private const long OptimizationFailure64BitValue = Int64.MaxValue; // 0x0100000000000000
		...
		public void WriteObject(object value)
		{
			...
			else
			{
                try
                {
                    ...
                    createBinaryFormatter().Serialize(streamTmp, value);
                    //Si pas d'erreur lors de la serialisation, écrit les bits dans le BaseStream
                    writeTypeCode(SerializedType.OtherType);
                    //BaseStream.Write(streamTmp.GetBuffer(), 0, streamTmp.GetBuffer().Length);
