...
{
    /// <summary>
    /// Récupère une données cumulée précalculéeà partir d'un
    /// <see cref="CTypeDonneeCumulee">Type de donnée cumulé</see>
    /// </summary>
    [Serializable]
    [AutoExec("Autoexec")]
    public class C2iExpressionGetDonneeCumulee : C2iExpressionAnalysable
    {
        /// //////////////////////////////////////////
        public C2iExpressionGetDonneeCumulee()
        {
        }
        ...
        {
            CAllocateur2iExpression.Register2iExpression(new C2iExpressionGetDonneeCumulee().IdExpression,
                typeof(C2iExpressionGetDonneeCumulee));
        }

        /// //////////////////////////////////////////
        protected override CInfo2iExpression GetInfosSansCache()
        {
            CInfo2iExpression info = new CInfo2iExpression(0, "GetCumulatedData",
                new CTypeResultatExpression(typeof(double), true),
                I.TT(GetType(), "GetCumulatedData([NoReadInDb],Code, nValue, key1, key2, ...) returns the value N°nValue (indexed on 0) of cumulated data according to the CODE type, for specified keys values.If NoReadInDb is set to true(), data will be read in local context only|217"),
