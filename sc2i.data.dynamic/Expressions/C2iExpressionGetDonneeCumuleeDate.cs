...
namespace sc2i.data.dynamic
{
    ...
    [Serializable]
    [AutoExec("Autoexec")]
    public class C2iExpressionGetDonneeCumuleeDate : C2iExpressionAnalysable
    {
        /// //////////////////////////////////////////
        public C2iExpressionGetDonneeCumuleeDate()
        {
        }
        ...
        {
            CAllocateur2iExpression.Register2iExpression(new C2iExpressionGetDonneeCumuleeDate().IdExpression,
                typeof(C2iExpressionGetDonneeCumuleeDate));
        }

        /// //////////////////////////////////////////
        protected override CInfo2iExpression GetInfosSansCache()
        {
            CInfo2iExpression info = new CInfo2iExpression(0, "GetCumulatedDateValue",
                new CTypeResultatExpression(typeof(double), true),
                I.TT(GetType(), "GetCumulatedDateValue([NoReadInDb], Code, nValue, key1, key2, ...) returns the date value nValue (indexed on 0) of cumulated data according to the CODE type, for specified keys values. If NoReadInDb is set to true(), data will be read in local context only|10012"),
        ...
        public override CResultAErreur MyEval(CContexteEvaluationExpression ctx, object[] listeParametres)
        {
            ...
            try
            {
                ...
                if (liste.Count > 0)
                {
                    ...
                    if (nParam < 0 || nParam > CParametreDonneeCumulee.c_nbChampsDate)
                    {
                        result.EmpileErreur(I.T("The cumulated data has no value @1|219", nParam.ToString()));
                        return result;
                    }
                    if (donnee.Row[CDonneeCumulee.c_baseChampDate + nParam.ToString()] == DBNull.Value)
                        result.Data = null;
                    else
                        result.Data = (DateTime?)donnee.Row[CDonneeCumulee.c_baseChampDate + nParam.ToString()];
                }
                else
                    result.Data = null; ;
            }
