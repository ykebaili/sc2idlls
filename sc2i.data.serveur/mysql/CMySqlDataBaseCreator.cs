...
{
	public class CMySqlDataBaseCreator : C2iDataBaseCreator
	{
		//------------------------------------------------------------------------------------------------
		...
		public override CResultAErreur CreateDatabase()
		{
			...
			if (!Connection.IsConnexionValide())
			{
				...
				MySqlConnection oraCon = new MySqlConnection(strConnexionString);
				try
				{
					oraCon.Open();
					MySqlCommand command = new MySqlCommand("Create database " + strBase, oraCon);
				...
				((CMySqlDatabaseConnexion)Connection).GetConnexion().Open();
		...
		public override CResultAErreur InitialiserDataBase()
		{
			...
			if(TableExists(CMySqlDatabaseConnexion.c_nomTableSysChampAuto))
				result = DeleteTable(CMySqlDatabaseConnexion.c_nomTableSysChampAuto);
			...
			if (result)
			{
				...
				result = m_connection.RunStatement(String.Format("Create table {0} ("
												...
												CMySqlDatabaseConnexion.c_nomTableSysChampAuto,
												CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomTable,
												CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomChamp,
												CMySqlDatabaseConnexion.c_nomChampSysChampAutoTriggerName,
												CMySqlDatabaseConnexion.c_nomChampSysChampAutoSeqName));
				//Creation des sequences de numérotation
				if (result)
					result = CreerSequencesNumerotationTable(CMySqlDatabaseConnexion.c_nomTableSysChampAuto);
		...
		{
			CResultAErreur result = CResultAErreur.True;
			if (strNomTableInDb == CMySqlDatabaseConnexion.c_nomTableSysChampAuto || 
				!TableExists(CMySqlDatabaseConnexion.c_nomTableSysChampAuto))
				return result;
		...
		internal CResultAErreur CreerSystemeAutoIncremente(string strNomTableInDb, string strNomChamp)
		{
            ...
            try
            {
                Type tp = CContexteDonnee.GetTypeForTable ( ((CMySqlDatabaseConnexion)Connection).GetNomTableInContexteFromNomTableInDb ( strNomTableInDb ));
			...
			if (result)
			{
				string strRefSysChampAutoExist = "SELECT * FROM "+
					CMySqlDatabaseConnexion.c_nomTableSysChampAuto+" WHERE "
					+CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomTable+" = '"
					+ strNomTableInDb + "' AND "
					+CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomChamp+" = '" 
					+ strNomChamp+"'";
                ...
                {
                    //Referencement
                    strUpdateRefSysChampAuto = "INSERT INTO " + CMySqlDatabaseConnexion.c_nomTableSysChampAuto +
                                        "(" +
                                        CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomTable + "," +
                                        CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomChamp + "," +
                                        CMySqlDatabaseConnexion.c_nomChampSysChampAutoSeqName + "," +
                ...
                else
                {
                    strUpdateRefSysChampAuto = 
                        "UPDATE "+CMySqlDatabaseConnexion.c_nomTableSysChampAuto+" set "+
                        CMySqlDatabaseConnexion.c_nomChampSysChampAutoSeqName+" = '"+strNomSeq+"', "+
                        CMySqlDatabaseConnexion.c_nomChampSysChampAutoTriggerName+" = '"+strNomTrigger+"' where "+
                        CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomTable+"='"+strNomTableInDb+"' and "+
                        CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomChamp+"='"+strNomChamp+"'";
		...
		internal CResultAErreur SupprimerSystemeAutoIncremente(string strNomTableInDb, string strNomChamp)
		{
            ...
            string strColAuto = "SELECT " +
                CMySqlDatabaseConnexion.c_nomChampSysChampAutoTriggerName + "," +
                CMySqlDatabaseConnexion.c_nomChampSysChampAutoSeqName + " from " +
                CMySqlDatabaseConnexion.c_nomTableSysChampAuto +
		...
		internal bool SystemeAutoIncrementeExist(string strNomTable, string strNomChamp)
		{
            ...
            string strColAuto = "SELECT TG_NAME, SQ_NAME FROM " + CMySqlDatabaseConnexion.c_nomTableSysChampAuto +
                    " WHERE " +
                    CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomTable + " = '" +
                    strNomTable + "' AND " +
                    CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomChamp + " = '" +
		...
		internal bool IsChampAutoIncremente(string strNomTable, string strNomChamp)
		{
			...
			string strColAuto = "SELECT "+
				CMySqlDatabaseConnexion.c_nomChampSysChampAutoTriggerName+","
				+CMySqlDatabaseConnexion.c_nomChampSysChampAutoSeqName+" FROM "+CMySqlDatabaseConnexion.c_nomTableSysChampAuto
				+ " WHERE "
				+CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomTable+" = '" 
				+ strNomTable + "' AND "
				+CMySqlDatabaseConnexion.c_nomChampSysChampAutoNomChamp+" = '" 
		...
		protected override string GetRequeteCreateIndex(string strNomTable, bool bCluster, params string[] strFields)
		{
			...
			string strNomTableSpaceIndex = ((CMySqlDatabaseConnexion)Connection).NomTableSpaceIndex;
        ...
        /// <summary>
        /// Pas de gestion d'index cluster dans MySql pour le moment
        /// </summary>
