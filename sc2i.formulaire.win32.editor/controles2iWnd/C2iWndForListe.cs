...
namespace sc2i.formulaire.win32
{
	...
	[AutoExec("Autoexec")]
	public partial class C2iWndForListe : UserControl, IControleWndFor2iWnd
	{
		...
		{
			CCreateur2iFormulaireV2.RegisterEditeur(typeof(C2iWndListe), typeof(C2iWndForListe));
		}
		...
		public void Remplir()
		{
			...
			try
			{
				...
				DataTable table = m_2iList.GetDataTable(datas, m_fournisseur);
		...
		private void m_linkForGrid_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			...
			if (nRow >= 0 && nRow < table.Rows.Count)
			{
				...
				{
					CResultAErreur result = CExecuteurActionSur2iLink.ExecuteAction(colonne.ActionSurLink, obj);
					if (!result)
