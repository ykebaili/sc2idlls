...
{
	public partial class CWndFor2iZoneMultipleCustomList : UserControl, IControleWndFor2iWnd, IControlALockEdition
	{
        ...
        private CPanelChildElementCustomList m_controlItem = null;


		public CWndFor2iZoneMultipleCustomList()
		{
			InitializeComponent();
            m_controlItem = new CPanelChildElementCustomList();
            m_panelSousFormulaires.ItemControl = m_controlItem;
            m_gestionnaireModeEdition.SetModeEdition(m_controlItem, TypeModeEdition.EnableSurEdition);
		}
        ...
        public void SetElementEditeSansChangerLesValeursAffichees(object elementEdite)
        {
            ...
            {
                CPanelChildElementCustomList sousFormulaire = ctrl as CPanelChildElementCustomList;
        ...
        private bool AffichePage( int nNumPage, bool bMajChampsAvantAffichage )
        {
            ...
            List<CItemChildZone> listeItems = new List<CItemChildZone>();
            ...
            for(int i = m_nPageEnCours * m_nbParPage; i < (m_nPageEnCours * m_nbParPage) + m_nbParPage; i++)
            {
                if (i < m_lstObjets.Count)
                {
                    ...
                    CItemChildZone item = new CItemChildZone();
                    item.Tag = data;
                    listeItems.Add(item);
                }
            }
            m_panelSousFormulaires.Items = listeItems.ToArray();
            m_panelSousFormulaires.Refill();
        ...
        private void RecalcAutoSize()
        {
            ...
            if (m_zoneMultiple.AutoSize )
            {
                ...
                foreach (CItemChildZone item in m_panelSousFormulaires.Items)
                {
                    if (m_zoneMultiple.Orientation == EOrientaion.Vertical)
                        nAutoSize += m_controlItem.GetItemHeight(item);
                    else
                        nAutoSize += m_controlItem.Width;
                }
                if (m_zoneMultiple.Orientation == EOrientaion.Vertical &&
		...
		{
			CResultAErreur result = CResultAErreur.True;
            result = m_controlItem.MajChamps();
			return result;
		}
                ...
                IGestionnaireReadOnlySysteme gestionnaireReadOnly)
		{
            ...
            if (m_zoneMultiple != null && m_zoneMultiple.SourceFormula != null)
            {
                ...
                if (tp != null)
                {
                    ...
                    m_controlItem.AppliqueRestrictions ( rest, listeRestriction, gestionnaireReadOnly );
                    //reste le delegue suppression
                }
            }
		}
        ...
        public void OnChangeParentModeEdition(bool bModeEdition)
        {
            ...
            m_controlItem.OnChangeParentModeEdition(!LockEdition);
            /*foreach (Control ctrl in m_panelSousFormulaires.Controls)
            {
                CPanelChildElementCustomList sousFormulaire = ctrl as CPanelChildElementCustomList;
                if (sousFormulaire != null)
