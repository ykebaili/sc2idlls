...
namespace sc2i.win32.common.folderbrowser
{
    #region Delegates
    ...
    /// </summary>
    public partial class CFileExplorer : UserControl
    {
        #region Fields
        ...
        private string m_strLastDisplayedFolderPath = null;
        private string m_strLastDisplayedFolderKey = null;
        ...
        void folderView_AfterSelect(object sender, TreeViewEventArgs e)
        {
            if (selectionChange)
            {
                ...
                if (e.Node != null)
                    m_strLastDisplayedFolderKey = GetFolderKey(e.Node);
        ...
        public string CurrentPath
        {
            get
            {
                ...
                if (node != null)
                {
                    ShellItem item = node.Tag as ShellItem;
                    if (item != null && item.IsFolder)
                        return item.Path;
                }
                return m_strLastDisplayedFolderPath;
            }
            set
            {
        ...
        public string CurrentFolderKey
        {
            get
            {
                TreeNode node = folderView.SelectedNode;
                ...
                if (node != null)
                {
                    return GetFolderKey(node);
                }
                return m_strLastDisplayedFolderKey;
            ...
            set
            {
                if (value != null)
                {
                    string[] strFolders = value.Split('\\');
                    TreeNode nodeSel = SelectFolderFromLabels(folderView.Nodes, strFolders, 0);
                    if (nodeSel != null)
                    {
                        folderView.SelectedNode = nodeSel;
                        return;
                    }
        ...
        private TreeNode SelectFolderFromLabels(TreeNodeCollection nodes, string[] strFolders, int nStartAt)
        {
            if (nStartAt >= strFolders.Length)
                return null;
            string strSearch = strFolders[nStartAt].ToUpper();
            foreach (TreeNode child in nodes)
            {
                if (child.Text.ToUpper() == strSearch)
                {
                    ExtendTreeNode(child, false);
                    TreeNode node = SelectFolderFromLabels(child.Nodes, strFolders, nStartAt + 1);
                    if (node != null)
                        return node;
                    return child;
                }
        ...
        public TreeNode SelectPath(string path, bool expandNode)
        {
            if (!string.IsNullOrEmpty(path) && PathExists(path))
            {
                ...
                {
                    pathParts = converted.Substring(2).Split('\\');
                    pathParts[0] = "\\" + pathParts[0];
                }
                else
                    pathParts = converted.Split('\\');
        ...
        private string GetFolderKey ( TreeNode node )
        {
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            if (newItem != null && newItem.IsFolder)
                m_strLastDisplayedFolderPath = newItem.Path;
            ...
            if (oldItem != newItem && newItem.Expand(true, false, Handle))
            {
                ...
                {
                    bool used = false;
                    foreach (CFileExplorer br in ShellBrowser.Browsers)
                    {
        ...
        private void fileView_MouseMove(object sender, MouseEventArgs e)
        {
            if ( e.Button == MouseButtons.Left && 
                m_ptStartDrag != null &&
                (Math.Abs ( m_ptStartDrag.Value.X - e.X ) > 3 ||
                Math.Abs(m_ptStartDrag.Value.Y - e.Y) > 3 ))
            {
                if ( fileView.SelectedItems.Count > 0 )
                {
                    string[] lstFiles = SelectedFiles;
                    if ( lstFiles != null && lstFiles.Length> 0 )
                    {
                            DoDragDrop(lstFiles, DragDropEffects.Link);
                    }
                }
            }
        }
        ...
        public string[] SelectedFiles
        {
            get
            {
                ...
                foreach (ListViewItem item in fileView.SelectedItems)
                {
                    ...
                    if (si != null && si.IsFileSystem)
                        lstFiles.Add(si.Path);
                }
